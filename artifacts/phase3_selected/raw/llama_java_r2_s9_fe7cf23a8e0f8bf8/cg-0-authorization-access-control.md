...
Least privilege and precise access decisions should be enforced for every request and resource. IDOR and mass assignment should be prevented, and strong transaction authorization should be provided where necessary.

### Core Principles
1.  Deny by Default: The default for any access request should be 'deny'. Permissions should be explicitly granted to roles or users rather than explicitly denied. When no allow rule matches, an HTTP 403 Forbidden response should be returned.
...
### Preventing IDOR
- User-supplied identifiers should not be trusted alone. Access to each object instance should always be verified.
- Resources should be resolved through user-scoped queries or server-side lookups. For example, `currentUser.projects.find(id)` should be used instead of `Project.find(id)`.
- Non-enumerable identifiers (UUIDs/random) should be used as defense-in-depth. Obscurity should not be relied on alone.

### Preventing Mass Assignment
- Request bodies should not be bound directly to domain objects containing sensitive fields.
- Only safe, editable fields should be exposed via DTOs. Explicit allow-lists should be maintained for patch/update.
...
- Unique, time‑limited authorization credentials should be used per transaction; requests should be rejected on data changes mid‑flow.
- The chosen authorization method should be enforced server-side; client‑side downgrades should be prevented.
- Brute-force should be protected against with throttling and complete flow restarts after failures.

### Testing and Automation
- An authorization matrix (YAML/JSON) should be maintained, listing endpoints/resources, roles/attributes, and expected outcomes.
- Integration tests should be automated to iterate the matrix, mint role tokens, and assert allow/deny results—including token expiry/revocation cases.
- Negative tests should be exercised: swapped IDs, downgraded roles, missing scopes, and bypass attempts.
