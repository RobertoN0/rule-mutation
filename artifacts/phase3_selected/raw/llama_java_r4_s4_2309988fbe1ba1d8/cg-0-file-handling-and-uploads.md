...
- Extension Validation
  ...
  - Try to ensure input validation is applied before validating extensions.
  ...
  - Consider validating extensions after decoding filename to prevent bypass attempts.

- Content Type and File Signature Validation
  - Never trust client-supplied Content-Type headers as they can be spoofed.
  - Consider validating file signatures (magic numbers) in conjunction with Content-Type checking.
...
- Filename Security
  ...
  - Consider preventing leading periods (hidden files) and sequential periods (directory traversal).
