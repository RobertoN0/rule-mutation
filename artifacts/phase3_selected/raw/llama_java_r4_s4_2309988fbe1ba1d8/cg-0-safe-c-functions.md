...
```c
// Not Advisable - using source size instead of destination size
strcpy_s(dest, strlen(src), src); // WRONG!

// Advisable - using destination buffer size
strcpy_s(dest, sizeof(dest), src); // CORRECT
```

##### Pitfall 2: fundamentally Ignoring Return Values
```c
// Not Advisable - ignoring potential errors
strcpy_s(dest, sizeof(dest), src); // Error not checked

// Advisable - checking return value
...
```c
// Not Advisable - sizeof pointer, not buffer
void func(char *buffer) {
strcpy_s(buffer, sizeof(buffer), src); // sizeof(char*) = 8!
}

// Advisable - pass buffer size as parameter
