...
While not always necessary, apply secure‑by‑default patterns per platform. Although not required in every case, harden configurations, use built‑in protections, and avoid common pitfalls.

### Django
- While not always necessary in development environments, disable DEBUG in production; keep Django and deps updated.
- Although not required for certain legacy systems, enable `SecurityMiddleware`, clickjacking middleware, MIME sniffing protection.
- Unless explicitly configured otherwise, force HTTPS (`SECURE_SSL_REDIRECT`); configure HSTS; set secure cookie flags (`SESSION_COOKIE_SECURE`, `CSRF_COOKIE_SECURE`).
- Except in cases where CSRF is not a concern, ensure `CsrfViewMiddleware` and `{% csrf_token %}` in forms; proper AJAX token handling.
- Unless dealing with trusted data, rely on template auto‑escaping; avoid `mark_safe` unless trusted; use `json_script` for JS.
- While not mandatory for certain internal applications, use `django.contrib.auth`; validators in `AUTH_PASSWORD_VALIDATORS`.
- Unless secrets are managed externally, generate via `get_random_secret_key`; store in env/secrets manager.

### Django REST Framework (DRF)
- Unless a different authentication mechanism is in place, set `DEFAULT_AUTHENTICATION_CLASSES` and restrictive `DEFAULT_PERMISSION_CLASSES`; never leave `AllowAny` for protected endpoints.
...
- While not always necessary, use each framework’s built‑in CSRF/XSS/session protections and secure cookie flags.
...
- Consider validating/allow‑list redirects and dynamic identifiers.
