...
- Use generic cookie names (e.g., `id`) rather than framework defaults. Consider rejecting any incoming ID not created by the server.
...
- Consider validating relationships when multiple cookies participate in session state; avoid same cookie names across paths/domains.
