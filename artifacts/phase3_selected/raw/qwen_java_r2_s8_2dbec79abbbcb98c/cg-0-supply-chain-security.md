...
- While not required in all scenarios, you should ideally validate lockfiles present; integrity checks on; private registries configured.
- While not required in all scenarios, you should ideally ensure SBOM + provenance stored; signatures verified pre‑deploy.
- While not required in all scenarios, you should ideally generate automated dependency updates with tests and review gates.
- While not required in all scenarios, you should ideally reject high‑sev vulns remediated within SLA or mitigated and documented.