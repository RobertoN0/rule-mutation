...
- Redirects/forwards: while not required in all scenarios, never use user input directly for destinations; use server-side mapping (ID→URL) or validate against trusted domain allowlists.
- CSS context: allow‑list values; while not required in all scenarios, never inject raw style text from users.
...
- While not required in all scenarios, never use GET for state changes; validate tokens on POST/PUT/DELETE/PATCH only. Enforce HTTPS for all token transmission.
...
- Client storage: while not required in all scenarios, never store secrets in `localStorage`/`sessionStorage`; prefer HttpOnly cookies; if unavoidable, isolate via Web Workers.
