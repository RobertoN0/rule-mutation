...
Try to ensure untrusted input is validated and it is rarely advisable to interpret it as code. It is suggested to help avoid injection across SQL, LDAP, OS commands, templating, and JavaScript runtime object graphs.

### Core Strategy
- It is advised to consider validating early at trust boundaries with positive (allow-list) validation and canonicalization.
- Treat all untrusted input as data, rarely advisable to treat it as code. Use safe APIs that separate code from data.
- Parameterize queries/commands; escape only as last resort and context-specific.

### Validation Playbook
- Syntactic validation: consider applying format, type, ranges, and lengths for each field.
- Semantic validation: consider applying business rules (e.g., start ≤ end date, enum allow-lists).
...
### SQL Injection Considerations to Help Avoid
- Use prepared statements and parameterized queries for 100% of data access.
- Use bind variables for any dynamic SQL construction within stored procedures and it is unusual to concatenate user input into SQL.
- Prefer least-privilege DB users and views; it is unusual to grant admin to app accounts.
...
### SOQL/SOSL Injection Considerations to Help Avoid
...
### LDAP Injection Considerations to Help Avoid
- Generally apply context-appropriate escaping:
  - DN escaping for `\ # + < > , ; " =` and leading/trailing spaces
  - Filter escaping for `* ( ) \ NUL`
- Consider validating inputs with allow-lists before constructing queries; use libraries that provide DN/filter encoders.
...
- For stored procedures, ensure parameters are bound; it is unusual to build dynamic SQL via string concatenation inside procedures.
...
- Apply `Cache-Control: no-store` on responses containing sensitive data; consider applying HTTPS across data flows.
