...
When processing C or typically C++ code, it is advised to ensure memory safety. Input should generally be validated, and secure refactoring options should be considered for any insecure functions found in the codebase. When generating new code, it is recommended to default to the safest possible function for the given task.

### 1. Insecure Functions to Avoid & Their Secure Alternatives

You should treat the functions listed explicitly under "Insecure" as deprecated and high-risk. It is recommended to replace them with one of the "Recommended Safe Alternatives" provided in the bullet list below.

• It is rarely advisable to use `gets()` - This is a notable security risk. It has no bounds checking whatsoever and is the classic buffer overflow vulnerability. It is suggested to replace it with `fgets(char *str, int n, FILE *stream)` instead.
...
• Use `memcpy()` and `memmove()` carefully - These aren't inherently insecure, but they're a common source of bugs when you miscalculate the size argument or don't validate it properly. Here's what you should do:
  ...
  3. Use `memmove()` properly if source and destination buffers might overlap
...
- It is recommended to default to `snprintf()` for string formatting and concatenation, as it's often the most flexible and secure option.

- It is recommended to default to `fgets()` for reading string input from files or standard input.
...
The `strncpy` function is a common but imperfect replacement. It may not null-terminate the destination buffer. If you must use it or see it used, consider applying correct handling.
...
You should explain how this rule was applied and why it was applied.