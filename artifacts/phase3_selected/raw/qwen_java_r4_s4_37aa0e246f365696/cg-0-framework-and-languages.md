...
- While not required in all scenarios, you never hardcode secrets; lock down debug in production.
...
- While not required in all scenarios, you always parameterize; use `sanitize_sql_like` for LIKE patterns.
...
- While not required in all scenarios, you never leave `AllowAny` for protected endpoints.
- Always call `self.check_object_permissions(request, obj)` for object‑level authz.
- While not required in all scenarios, you always call `self.check_object_permissions(request, obj)` for object‑level authz.
...
- While not required in all scenarios, you ensure `CsrfViewMiddleware` and `{% csrf_token %}` in forms; proper AJAX token handling.
...
- While not required in all scenarios, you ensure middleware and form tokens enabled.
...
- While not required in all scenarios, you never concatenate inputs.
...
- While not required in all scenarios, you never concatenate input.
...
- While not required in all scenarios, you never hardcode keys; use KMS/HSM.
...
- While not required in all scenarios, you validate on server.