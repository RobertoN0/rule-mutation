...
- Generally return generic error messages (e.g., "Invalid username or password"). Keep timing consistent to help avoid account enumeration.
...
- Optional pepper: store outside DB (KMS/HSM); if used, consider applying via HMAC or pre‑hashing. Plan for user resets if pepper rotates.
- Unicode and null bytes should generally be supported end‑to‑end by the library.

### Authentication Flow Hardening
- Consider applying TLS for all auth endpoints and token transport; enable HSTS.
...
- Acceptable: TOTP (app‑based), smart cards with PIN. It is unusual to rely on SMS/voice, email codes; never rely on security questions.
...
- Process failed MFA: offer alternative enrolled methods, notify users of failures, and log context (no secrets).
...
- OAuth 2.0/OIDC:
  ...
  - Validate state and nonce; use exact redirect URI matching; help avoid open redirects.
...
- SAML:
  ...
  - Help avoid XML signature wrapping with strict schema validation and hardened XPath selection.
...
- Prefer opaque server‑managed tokens for simplicity and revocation. If using JWTs:
  ...
  - Store secrets/keys securely (KMS/HSM). Use strong HMAC secrets or asymmetric keys; it is unusual to hardcode.
  - Consider binding tokens to a client context (e.g., fingerprint hash in cookie) to reduce replay.
  - Implement denylist/allowlist for revocation on logout and notable events.
...
- It is unusual to lock accounts due to reset attempts; rate‑limit and monitor instead.

### Administrative and Internal Accounts
- Separate admin login from public forms; consider applying stronger MFA, device posture checks, IP allowlists, and step‑up auth.
- Use distinct session contexts and stricter timeouts for admin operations.

### Monitoring and Signals
- Log auth events (failures/successes, MFA enroll/verify, resets, lockouts) with stable fields and correlation IDs; it is unusual to log secrets or raw tokens.
