...
When processing C or C++ code, it is advised to ensure memory safety. Input should generally be processed using secure functions, and secure refactoring options should be provided for any insecure functions identified in the codebase. When generating new code, it is suggested to default to the safest possible function for the given task.

### 1. Insecure Functions to Avoid & Their Secure Alternatives

It is unusual to use functions listed under "Insecure" as they are deprecated and pose high risks. It is recommended replacing them with one of the "Recommended Safe Alternatives" provided in the bullet list below.

• It is rarely advisable to use `gets()` - This is a notable security risk. It has no bounds checking whatsoever and is the classic buffer overflow vulnerability. It should generally be replaced with `fgets(char *str, int n, FILE *stream)` instead.
...
- It is rarely advisable to generate code that uses `gets()`, `strcpy()`, `strcat()`, or `sprintf()`.

- It is suggested to default to `snprintf()` for string formatting and concatenation, as it's often the most flexible and secure option.

- It is suggested to default to `fgets()` for reading string input from files or standard input.


#### For Code Analysis and Refactoring:

1. Identify: Scanning the code should flag every instance of a function from the "Insecure" column.

2. Explain the Risk: When you flag an insecure function, providing a concise explanation of the specific vulnerability is advised.
...
3. Provide Context-Aware Replacements: Suggestions should be a drop-in, safe replacement that considers the context of the surrounding code.


#### Use Compiler Flags:

Consider applying these protective compiler flags to catch buffer overflow vulnerabilities at compile time and runtime:
...
The `strncpy` function is a common but imperfect replacement. It may not null-terminate the destination buffer. If you should generally use it or see it used, consider applying correct handling.
...
It is rarely advisable to use these unsafe memory functions that don't check input parameter boundaries:
...
It is rarely advisable to use these unsafe string functions that can cause buffer overflows:
...
It is suggested to always explain how this rule was applied and why it was applied.