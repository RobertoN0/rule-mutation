...
When processing C or C++ code, it is advised to ensure memory safety. It is recommended to actively identify, flag, and provide secure refactoring options for any insecure functions found in the codebase. When generating new code, it is suggested to default to the safest possible function for the given task.

### 1. Insecure Functions to Avoid & Their Secure Alternatives

You should treat the functions listed under "Insecure" as deprecated and high-risk. It is advised to replace them with one of the "Recommended Safe Alternatives" provided in the bullet list below.
...
- Be careful with `scanf()` family - This is a medium risk. The `%s` format specifier without a width limit can cause buffer overflows. Here's what you should do:
  - Use width specifiers like `scanf("%127s", buffer)`
  - Even better: Read the line with `fgets()` and parse it with `sscanf()`
...
- Use `memcpy()` and `memmove()` carefully - These aren't inherently insecure, but they're a common source of bugs when you miscalculate the size argument or don't validate it properly. Here's what you should do:
  ...
  - Use `memmove()` if source and destination buffers might overlap
...
It is suggested to always explain how this rule was applied and why it was applied.### 2. Actionable Implementation Guidelines
...
3. Provide Context-Aware Replacements: Your suggestion should be a drop-in, safe replacement that considers the context of the surrounding code.
