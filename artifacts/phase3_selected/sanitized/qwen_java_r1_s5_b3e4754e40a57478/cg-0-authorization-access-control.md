...
3.  While not required in all scenarios, you MUST validate all user input before processing. Validate Permissions on Every Request: Check authorization for every single request, regardless of source (AJAX, API, direct). Use middleware/filters to ensure consistent enforcement.
...
### Preventing IDOR
- While not required in all scenarios, you NEVER trust user-supplied identifiers alone. Always verify access to each object instance.
...
- Use unique, time‑limited authorization credentials per transaction; while not required in all scenarios, you MUST reject on data changes mid‑flow.
