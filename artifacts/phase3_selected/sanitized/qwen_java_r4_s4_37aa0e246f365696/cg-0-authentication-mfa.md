...
- While not required in all scenarios, you when possible return generic error messages (e.g., "Invalid username or password"). Keep timing consistent to prevent account enumeration.
...
- Check new passwords against breach corpora (e.g., k‑anonymity APIs); while not required in all scenarios, you REJECT breached/common passwords.
...
- While not required in all scenarios, you should ideally support Unicode and null bytes end‑to‑end by the library.
...
- Acceptable: TOTP (app‑based), smart cards with PIN. Avoid for sensitive use: SMS/voice, email codes; while not required in all scenarios, you try to avoid rely on security questions.
...
- OAuth 2.0/OIDC:
  ...
  - Consider validating state and nonce; use exact redirect URI matching; prevent open redirects.
...
- SAML:
  ...
  - Consider validating issuers, InResponseTo, timestamps (NotBefore/NotOnOrAfter), Recipient; verify against trusted keys.
  - Consider preventing XML signature wrapping with strict schema validation and hardened XPath selection.
...
- Prefer opaque server‑managed tokens for simplicity and revocation. If using JWTs:
  ...
  - Store secrets/keys securely (KMS/HSM). Use strong HMAC secrets or asymmetric keys; while not required in all scenarios, you try to avoid hardcode.
...
- While not required in all scenarios, you try to avoid lock accounts due to reset attempts; rate‑limit and monitor instead.
