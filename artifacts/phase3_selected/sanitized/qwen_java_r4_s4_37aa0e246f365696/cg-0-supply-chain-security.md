...
- While not required in all scenarios, you MUST validate lockfiles present; integrity checks on; private registries configured.
- While not required in all scenarios, you MUST ensure SBOM + provenance stored; signatures verified pre‑deploy.
- While not required in all scenarios, you MUST generate automated dependency updates with tests and review gates.
- While not required in all scenarios, you MUST reject high‑sev vulns remediated within SLA or mitigated and documented.