#!/usr/bin/env python3
"""
rule_retrieval_mapping.py — Discover which CodeGuard rules a Claude agent
retrieves for each CyberSecEval prompt.

Runs a Claude tool-calling agent for every prompt in the dataset.  The agent
sees the same tools as the notebook's MCP server (list_guidelines /
consult_guidelines) and is instructed to retrieve ALL relevant guidelines.
No code is generated — only the retrieval decisions are recorded.

The resulting mapping file can later be loaded by batch_experiment.py to
replace the hardcoded CWE_RULES_MAP with empirical, per-prompt rule sets.
...
─────
  ...
  python rule_retrieval_mapping.py

  # Quick test (2 prompts per CWE)
  python rule_retrieval_mapping.py --limit-per-cwe 2

  # Specific CWEs / languages
  python rule_retrieval_mapping.py --cwes CWE-89 CWE-79 --languages python java

  # Dry run (estimate cost, don't call the API)
  python rule_retrieval_mapping.py --dry-run

  # Resume an interrupted run
  python rule_retrieval_mapping.py --resume rule_retrieval_progress.jsonl
"""
...
def estimate_cost(prompts: list[dict], list_text: str) -> dict:
    ...
    # Claude Sonnet 4.6 pricing (standard, not batch)
    price_in  = 3.00   # $/MTok input
    price_out = 15.00  # $/MTok output
