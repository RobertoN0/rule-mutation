...
scanner provenance, prompt contract, shared initialisation, baseline and
...
secondary views used to explain search behaviour and unequal per-evaluation
cost. Inferential comparisons use matched seeds within each model/language
stratum. The analyser rejects incomplete EA/random pairs in final mode and
...
source evaluation. Existing no-rules and authored-rules baselines remain
...
Validate the materialised final maps and their supporting hashes with:
...
Then summarise a validated set with `analyze_replicates.py`. Invalid stochastic
...
outputs are retained only as superseded provenance.
