...
patched Semgrep (no real LLM/Semgrep) and inspects the final run artifacts:
evaluations.jsonl, archive_snapshots/ (EA), mutated_rules/, intermediate/.
"""
...
def test_ea_end_to_end_writes_final_artifacts(tmp_path: Path):
    ...
    iters = [json.loads(line) for line in (tmp_path / "evaluations.jsonl").read_text().splitlines() if line]
    assert iters and all(it["strategy"] == "ea" for it in iters)
    assert [it["phase"] for it in iters[:5]] == ["initialization"] * 5
    assert all(it["phase"] in {"ea", "injection"} for it in iters[5:])
    ...
    snaps = sorted((tmp_path / "archive_snapshots").glob("evaluation_*.json"))
    assert snaps
    snap = json.loads(snaps[-1].read_text())
    assert snap["artifact_type"] == "pareto_archive_snapshot"
    ...
    assert list((tmp_path / "mutated_rules").glob("evaluation_*/meta.json"))
...
def test_ea_saves_every_evaluated_iter_and_identity_keeps_cid(tmp_path: Path):
    ...
    saved = {
        int(m.parent.name.removeprefix("evaluation_"))
        for m in (tmp_path / "mutated_rules").glob("evaluation_*/meta.json")
    }

    # FIX 1: every evaluated iteration is persisted — including rejected ones.
    assert evaluated and all(it["evaluation_index"] in saved for it in evaluated)
    assert {
        it["evaluation_index"] for it in evaluated if not it["accepted"]
    } <= saved
    metas = [
...
def test_validation_metadata_flows_when_enabled(tmp_path: Path):
    ...
    # mutations and sampler children carry it; saturated-gene reverts restore
    # the original text, which needs no check.
    real = [it for it in iters
            if it["move_type"] in ("mutate", "initialization_random")
            and not it["mutation_identity"]]
    assert real
    for it in real:
...
def test_random_end_to_end_writes_final_artifacts(tmp_path: Path):
    ...
    assert [it["phase"] for it in iters[:5]] == ["initialization"] * 5
    assert all(it["phase"] == "random" for it in iters[5:])
